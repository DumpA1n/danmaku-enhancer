...
**设置面板**支持两种放置方式(设置里切换):`内嵌原生`(挂进站点原生弹幕设置弹层)或
`独立浮窗`(平台无关的可拖动浮窗,默认收起成小标题条不遮挡界面,位置与展开状态持久化,
全屏时自动跟随)。多平台适配时浮窗更省事,无需为每个站点找原生弹层挂载点。
